...
from app.usermodels.models import UserAnswerChoice
from app.usermodels.models import UserUpdateAnswerChoice
from app.usermodels.models import UserAnswerText
from app.usermodels.models import UserUpdateAnswerText
    ...
    with SessionLocal() as session:
        for answer_id in body.answers:
            answer = session.query(QuestionAnswer).filter(
                QuestionAnswer.id == answer_id).first()
            correct = answer.answer_correct
            if answer.answer_points is not None:
                point = answer.answer_points
            else:
                point = 0
...
async def update_choice(
        response: Response,
        body: UserUpdateAnswerChoice,
        id: int):
    time = datetime.datetime.now()
    with SessionLocal() as session:
        us_choice_ans = session.query(QuestionUserAnswerChoice).filter(
            QuestionUserAnswerChoice.id == id).first()
...
async def remove_user_answer_choice_by_index(response: Response, id: int):
    with SessionLocal() as session:
        ...
        session.query(QuestionUserAnswerChoice).filter(
            QuestionUserAnswerChoice.id == id).delete()
...
async def add_text_ans(response: Response, body: UserAnswerText):
    time = datetime.datetime.now()
    session_id = body.session_id
    question_id = body.question_id
    with SessionLocal() as session:
...
async def update_text(response: Response, body: UserUpdateAnswerText, id: int):
    time = datetime.datetime.now()
    with SessionLocal() as session:
        text = session.query(QuestionUserAnswerText).filter(
            QuestionUserAnswerText.id == id).first()
        text.answer = body.answer
        text.answer_time = time
        return {"success": 1, "data": id}
...
async def remove_user_answer_text_by_index(response: Response, id: int):
    with SessionLocal() as session:
        us_text_ans = session.query(QuestionUserAnswerText).filter(
            QuestionUserAnswerText.id == id).first()
        if not (us_text_ans):
            response.status_code = 404
            return {"success": 0, "error": f"Ответ с id {id} не существует"}
        session.query(QuestionUserAnswerText).filter(
            QuestionUserAnswerText.id == id).delete()
