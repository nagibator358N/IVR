...
async def add_answer(response: Response, body:AddQuestionAnswer, id:int):
    with SessionLocal() as session:
        answer = session.query(QuestionAnswer).filter(QuestionAnswer.id==id).first()
        if answer:
             if body.answer_text!=None:
                 answer.answer_text =body.answer_text
             if body.answer_correct!=None:
                 answer.answer_correct = body.answer_correct 
             if body.answer_number!=None:
                 answer.answer_number = body.answer_number
             if body.answer_points!=None:
                 answer.answer_points = body.answer_points
             session.commit()
