...
from app.usermodels.models import UpdateQuizQuestion
...
async def update_question(
    ...
    with SessionLocal() as session:
        ...
        if question:
            ...
            if body.question_description is not None:
                question.question_description = body.question_description
            if body.question_time is not None:
                question.question_time = body.question_time
            if body.question_points is not None:
            ...
            if body.question_difficulty is not None:
                question.question_difficulty = body.question_difficulty
            if body.question_hint is not None:
...
async def get_question_by_id(response: Response, id: int):
    with SessionLocal() as session:
        question = session.query(QuizQuestion).filter(
            QuizQuestion.id == id).order_by(asc(QuizQuestion.question_number)
                                            ).first()
        if not (question):
            response.status_code = 404
            return {"success": 0, "error": f"Вопрос с id {id} не существует"}
...
async def get_answer_list(response: Response, id: int):
    with SessionLocal() as session:
        ...
        list_answers = session.query(QuestionAnswer).filter(
            QuestionAnswer.question_id == id).order_by(asc(
                QuestionAnswer.answer_number)).all()
        result = []
